...
if (app_is_logged_in()) {
  ...
  if ($uid > 0) {
    $existingProfile = app_load_profile($pdo, $uid, 'kaigo') ?: app_load_profile($pdo, $uid, 'basic');
    if ($existingProfile && $next === '/rireki/user_data.php') {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($mode === 'register') {
    ...
    if (!$row || !password_verify($pass, (string)$row['password_hash'])) {
      ...
      // If user has an existing profile and didn't specify a form url, send them straight to dashboard
      $existingProfile = app_load_profile($pdo, $uid, 'kaigo') ?: app_load_profile($pdo, $uid, 'basic');
      if ($existingProfile && $next === '/rireki/user_data.php') {
        header('Location: /rireki/user_data.php', true, 302);
      } else {
      ...
      <div class="auth-card">
        ...
        <div class="card-body">
          ...
          <div class="pane <?php echo ($tab==='login'?'active':''); ?>" id="paneLogin" role="tabpanel" aria-labelledby="tabLogin">
            <form method="post" action="/rireki/rireki_login.php?next=<?= h($next) ?>">
          ...
          <div class="pane <?php echo ($tab==='register'?'active':''); ?>" id="paneRegister" role="tabpanel" aria-labelledby="tabRegister">
            <form method="post" action="/rireki/rireki_login.php?next=<?= h($next) ?>" id="regForm">
