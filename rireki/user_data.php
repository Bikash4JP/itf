...
$kToken = null;
if ($profKaigo && !empty($profKaigo['token'])) {
  $kToken = $profKaigo['token'];
}
if (!$kToken) {
  $stKaigo = $pdo->prepare("SELECT token FROM app_resume_kaigo WHERE user_id = ? ORDER BY id DESC LIMIT 1");
  $stKaigo->execute([$uid]);
  $kToken = $stKaigo->fetchColumn();
}

$profBasic = app_load_profile($pdo, $uid, 'basic');
$hasKaigo = $kToken !== false && $kToken !== null;
$hasBasic = $profBasic !== null;

// Determine primary format to show
if ($hasKaigo) {
  header("Location: /rireki/kaigo/php/rireki_preview.php?flow=profile_only&token=" . urlencode((string) $kToken), true, 302);
  exit;
}
...
if ($hasBasic) {
  ...
  $t = $st->fetchColumn();
  if ($t)
    $editUrl .= '?token=' . urlencode((string) $t);
}
...
<body>
  ...
  <nav class="topnav">
    <div class="nav-left">
      <a href="/rireki/" class="nav-logo-mark"><img src="https://it-future.jp/images/android-chrome-192x192.png"
          alt="ITF"></a>
      <div>
