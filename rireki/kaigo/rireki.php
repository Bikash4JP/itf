...
function map_post_to_columns(array $post): array
{
  ...
  foreach ($direct as $k) {
    if (array_key_exists($k, $post))
      $out[$k] = is_string($post[$k]) ? trim($post[$k]) : $post[$k];
  }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $action === 'upload_photo') {
  ...
  elseif ($mime === 'image/png')
    ...
    json_out(['ok' => false, 'error' => 'only JPEG/PNG allowed'], 400);
...
<head>
  ...
  <style>
    ...
    *::after {
    ...
    #progressWrap .progress-labels li.is-active {
      opacity: 1;
      color: #7dc8ff;
    }

    #progressWrap .progress-labels li.is-done {
      opacity: .85;
      color: var(--ok);
    }
  ...
  <div class="appbar">
    <div class="wrap">
      <a class="nav-logo" href="/rireki/">
        <div class="nav-logo-mark"><img src="https://it-future.jp/images/android-chrome-192x192.png" alt="ITF"
      ...
      <div class="steps">
        ...
        <section class="step-pane is-active" d="step-1">
          ...
          <div class="grid-2">
            <label>
              ...
              <input type="text" name="name_romaji" id="name_romaji" placeholder="TARO YAMADA" required
                autocomplete="name" value="<?php echo h($row['name_romaji'] ?? ''); ?>">
              <small class="hint">※ 英字は大文字（A-Z）のみ / または漢字で入力してください。</small>
            </label>

            <label>
              <span class="lbl">氏名（カタカナ）<span class="req">*</span></span>
              <input type="text" name="name_kana" id="name_kana" placeholder="ヤマダ タロウ" required autocomplete="name"
            ...
            <div>
              ...
              <div class="grid-3">
                <input class="in-yy" type="text" name="dob_year" id="dob_year" placeholder="YYYY" inputmode="numeric"
                  required maxlength="4" pattern="\d{4}" value="<?php echo h($row['dob_year'] ?? ''); ?>">
                <input class="in-mm" type="text" name="dob_month" id="dob_month" placeholder="MM" inputmode="numeric"
                  required maxlength="2" pattern="\d{1,2}" value="<?php echo h($row['dob_month'] ?? ''); ?>">
                <input class="in-mm" type="text" name="dob_day" id="dob_day" placeholder="DD" inputmode="numeric"
                  required maxlength="2" pattern="\d{1,2}" value="<?php echo h($row['dob_day'] ?? ''); ?>">
              </div>
            </div>
            ...
            <label>
              ...
              <input type="number" name="age_autofill" id="age_autofill" readonly
                value="<?php echo h($row['age_autofill'] ?? ''); ?>">
            </label>

            <label>
              <span class="lbl">出生地<span class="req">*</span></span>
              <input type="text" name="birthplace" placeholder="生まれた市と国を入力" required
                value="<?php echo h($row['birthplace'] ?? ''); ?>">
            </label>
            <div></div>

            <!-- ── 現住所 グループ ── -->
            <div
              style="grid-column:1/-1;border:1px solid rgba(59,158,255,.2);border-radius:12px;padding:14px 16px 8px;background:rgba(59,158,255,.06);margin:4px 0 0">
              <div
                style="font-weight:800;color:#7dc8ff;font-size:14px;margin-bottom:10px;display:flex;align-items:center;gap:6px">
                🏠 現住所
              </div>
              <div class="grid-2" style="gap:10px">
                <label>
                  ...
                  <div style="display:flex;gap:8px;align-items:center">
                    <input type="text" name="postal" id="postal" placeholder="1234567 (ハイフンなし)" inputmode="numeric"
                      required pattern="\d{3}-?\d{4}" maxlength="8" style="flex:1"
                      value="<?php echo h($row['postal'] ?? ''); ?>">
                    <button type="button" id="btnPostal"
                      style="padding:6px 12px;border-radius:8px;border:1px solid rgba(59,158,255,.3);background:rgba(59,158,255,.15);color:#7dc8ff;cursor:pointer;white-space:nowrap;font-weight:700;font-size:13px">住所検索</button>
                  </div>
                ...
                <label>
                  ...
                  <input type="text" name="address_auto" id="address_auto" placeholder="郵便番号を入力すると自動入力されます"
                    style="background:rgba(255,255,255,.03);color:var(--muted);border-color:rgba(255,255,255,.08)"
                    value="<?php echo h($row['address_auto'] ?? ''); ?>">
                </label>

                <label style="grid-column:1/-1">
                  <span class="lbl">丁目・番地・建物名・部屋番号<span class="req">*</span></span>
                  <input type="text" name="address" placeholder="例：1-2-3 ○○マンション 101号" required
                    autocomplete="street-address" value="<?php echo h($row['address'] ?? ''); ?>">
          ...
          <div class="grid-2">
            ...
            <label>
              ...
              <input type="text" name="past_travel_details" placeholder="例：2019/05 日本入国、2021/08 帰国 など"
                value="<?php echo h($row['past_travel_details'] ?? ''); ?>">
            </label>
            ...
            <label class="col-2">
              ...
              <div class="photo-preview" id="photoPreview"
                style="<?php echo $hasPhoto ? 'display:flex' : 'display:none'; ?>">
          ...
          <table class="table edu" id="eduTable">
            ...
            <tbody>
              <?php for ($i = 1; $i <= $eduRows; $i++): ?>
                <tr class="edu-row">
                  <td><input class="in-yy" type="text" name="education[from_year][]" placeholder="YYYY"
                      inputmode="numeric" value="<?php echo h($row["edu{$i}_from_year"] ?? ''); ?>"></td>
                  <td><input class="in-mm" type="text" name="education[from_month][]" placeholder="MM" inputmode="numeric"
                      value="<?php echo h($row["edu{$i}_from_month"] ?? ''); ?>"></td>
                  <td><input class="in-yy js-edu-end-y" type="text" name="education[to_year][]" placeholder="YYYY"
                      inputmode="numeric" value="<?php echo h($row["edu{$i}_to_year"] ?? ''); ?>"></td>
                  <td><input class="in-mm js-edu-end-m" type="text" name="education[to_month][]" placeholder="MM"
                      inputmode="numeric" value="<?php echo h($row["edu{$i}_to_month"] ?? ''); ?>"></td>
                  <td>
                  ...
                  <td><input type="text" name="education[institution][]" placeholder="△△大学 / ○○高校"
                      value="<?php echo h($row["edu{$i}_institution"] ?? ''); ?>"></td>
                  <td><input type="text" name="education[faculty][]" placeholder="情報学部 など"
                      value="<?php echo h($row["edu{$i}_faculty"] ?? ''); ?>"></td>
                  <td>
          ...
          <table class="table" id="licTable">
            ...
            <tbody>
              <?php for ($i = 1; $i <= $licRows; $i++): ?>
                <tr class="lic-row">
                  <td><input class="in-yy" type="text" name="licenses[cert_year][]" placeholder="YYYY" inputmode="numeric"
                      value="<?php echo h($row["lic{$i}_year"] ?? ''); ?>"></td>
                  <td><input class="in-mm" type="text" name="licenses[cert_month][]" placeholder="MM" inputmode="numeric"
                      value="<?php echo h($row["lic{$i}_month"] ?? ''); ?>"></td>
                  <td><input type="text" name="licenses[cert_name][]" placeholder="介護職員初任者研修 など"
                      value="<?php echo h($row["lic{$i}_name"] ?? ''); ?>"></td>
                  <td>
        ...
        <section class="step-pane" d="step-4">
          ...
          <table class="table work" id="expTable">
            ...
            <tbody>
              <?php for ($i = 1; $i <= $workRows; $i++): ?>
                <tr class="exp-row">
                  <td><input class="in-yy" type="text" name="work_blocks[from_year][]" placeholder="YYYY"
                      inputmode="numeric" value="<?php echo h($row["work{$i}_from_year"] ?? ''); ?>"></td>
                  <td><input class="in-mm" type="text" name="work_blocks[from_month][]" placeholder="MM"
                      inputmode="numeric" value="<?php echo h($row["work{$i}_from_month"] ?? ''); ?>"></td>
                  <td>
                  ...
                  <td><input class="in-yy js-exp-end-y" type="text" name="work_blocks[to_year][]" placeholder="YYYY"
                      inputmode="numeric" value="<?php echo h($row["work{$i}_to_year"] ?? ''); ?>"></td>
                  <td><input class="in-mm js-exp-end-m" type="text" name="work_blocks[to_month][]" placeholder="MM"
                      inputmode="numeric" value="<?php echo h($row["work{$i}_to_month"] ?? ''); ?>"></td>
                  <td><input type="text" name="work_blocks[org][]" placeholder="特養 / 老健 / 企業名"
                      value="<?php echo h($row["work{$i}_org"] ?? ''); ?>"></td>
                  <td><input type="text" name="work_blocks[job_title][]" placeholder="介護職 / 生活相談員 / 看護助手 等"
                      value="<?php echo h($row["work{$i}_job_title"] ?? ''); ?>"></td>
                  <td><textarea name="work_blocks[description][]" rows="4"
                      placeholder="主な業務内容・担当・実績などを簡潔に（〜100語目安）"><?php echo h($row["work{$i}_description"] ?? ''); ?></textarea>
                  </td>
                  <td>
          ...
          <label class="col-2" style="margin-top:10px;">
            ...
            <textarea name="reason_for_resignation" rows="3"
              placeholder="退職済みの場合のみ入力"><?php echo h($row['reason_for_resignation'] ?? ''); ?></textarea>
          </label>
