...
if ($photoPath !== '' && file_exists($_SERVER['DOCUMENT_ROOT'] . $photoPath)) {
  $photoHtml = '<img src="' . h($photoPath) . '" alt="写真" style="width:100%;height:100%;object-fit:cover;">';
}
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    html, body { width: 100%; background: #f0f0f0; font-family: 'MS Gothic','Meiryo','Hiragino Kaku Gothic ProN',sans-serif; font-size: 8.5pt; color: #000; }
    ...
    @media print {
      ...
      .page { width: 100%; margin: 0; padding: 4mm; box-shadow: none; }
    ...
    /* ── Layout grid ── */
    .top-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 2mm; margin-bottom: 2mm; }
    .col-right { display: flex; flex-direction: column; gap: 2mm; }
    ...
    .small-note { font-size: 6.5pt; color: #555; }

    /* ── Print header bar ── */
    .print-header { text-align: center; margin-bottom: 3mm; }
    .print-header h1 { font-size: 14pt; font-weight: bold; letter-spacing: 4mm; }
    .print-date { text-align: right; font-size: 8pt; margin-bottom: 2mm; }
    ...
    .no-print button {
      background: #3b9eff; border: none; color: #fff;
      padding: 8px 22px; border-radius: 8px; cursor: pointer; font-size: 14px; font-weight: 700;
    }
    .no-print button:hover { background: #1e78e8; }
    ...
    .photo-placeholder { color: #aaa; font-size: 7pt; text-align: center; }
  </style>
</head>
<body class="has-bar">
...
<div class="page">
  ...
  <div class="print-date"><?= h($today) ?>現在</div>
  ...
  <table>
    <tr>
      ...
      <td rowspan="8" style="width:25%; border:1px solid #333; padding:0; vertical-align:top;">
        <div class="photo-box" style="width:100%; height:50mm; float:none; margin:0;">
          <?php if ($photoHtml): echo $photoHtml; else: ?>
    ...
    <tr>
      ...
      <td class="val" colspan="3">
        ...
        <?php $pexp = trim(($row['passport_exp_year']??'').'年'.($row['passport_exp_month']??'').'月'.($row['passport_exp_day']??'').'日'); if (strlen($pexp) > 3): ?>
  ...
  <table>
    <tr>
      ...
      <td class="val num" style="width:15%"><?= h(val($row,'status_from_year')) ?>年 <?= h(val($row,'status_from_month')) ?>月 <?= h(val($row,'status_from_day')) ?>日</td>
      <td class="label" style="width:10%">（終了）</td>
      <td class="val num"><?= h(val($row,'status_to_year')) ?>年 <?= h(val($row,'status_to_month')) ?>月 <?= h(val($row,'status_to_day')) ?>日</td>
    </tr>
    <tr>
      <td class="label">最近の入国</td>
      <td class="val num"><?= h(val($row,'recent_entry_year')) ?>年 <?= h(val($row,'recent_entry_month')) ?>月 <?= h(val($row,'recent_entry_day')) ?>日</td>
      <td class="label">最近の出国</td>
      <td class="val num" colspan="3"><?= h(val($row,'recent_exit_year')) ?>年 <?= h(val($row,'recent_exit_month')) ?>月 <?= h(val($row,'recent_exit_day')) ?>日</td>
    </tr>
  </table>
  ...
  <table>
    ...
    <tbody>
      <?php for ($i = 1; $i <= 8; $i++): $e = edu_row($row, $i); ?>
  ...
  <table>
    ...
    <tbody>
      <?php for ($i = 1; $i <= 8; $i++): $l = lic_row($row, $i); ?>
  ...
  <table>
    ...
    <tbody>
      <?php for ($i = 1; $i <= 8; $i++): $w = work_row($row, $i); ?>
        <?php if ($w['org'] !== '' || $w['from'] !== ''): ?>
        <tr>
  ...
  <?php $rr = trim((string)($row['reason_for_resignation'] ?? '')); if ($rr !== ''): ?>
  ...
  <table>
    ...
    <tr>
      <th>喫煙</th><th>飲酒</th><th>タトゥー</th>
      <th>服のサイズ</th><th>靴のサイズ</th><th>仕事の継続希望</th>
    </tr>
    <tr>
    ...
    <tr>
      <th>お祈り</th><th>断食</th><th>食べ物の制限</th>
      <th>ヒジャブ</th><th>日本語勉強中</th><th>専門職勉強中</th>
    </tr>
    <tr>
    ...
    <?php $oai = trim((string)($row['other_agency_or_facility_interview'] ?? '')); if ($oai !== ''): ?>
    <tr>
  ...
  <table style="margin-top:3mm;">
    <tr>
      ...
      <td class="label" style="width:12%">本人署名</td>
      <td class="val" style="height:16mm;"></td>
    </tr>
  </table>
