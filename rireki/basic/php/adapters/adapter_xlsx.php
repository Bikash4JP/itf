<!-- rireki\basic\php\adapters\adapter_xlsx.php -->
<?php
...
function rireki_render_pdf(array $data, string $mappingFile, string $outDir, string $token): array
{
  try {
    ...
    try {
      $sheet->setCellValue('J3', (string) ($data['meta']['today_jp'] ?? ''));
    } catch (\Throwable $e) {
    }
    ...
    if (!empty($map['photo']) && !empty($data['photo_path'])) {
      ...
      // fallback: web path -> absolute fs path
      if (is_string($ph) && $ph !== '' && !is_readable($ph) && strpos($ph, '/') === 0) {
        $docRoot = rtrim($_SERVER['DOCUMENT_ROOT'] ?? '', '/');
        if ($docRoot !== '') {
          $candidate = realpath($docRoot . $ph);
          $allowBase = realpath($docRoot . '/rireki/uploads');
          if ($candidate && $allowBase && strpos($candidate, $allowBase) === 0 && is_readable($candidate)) {
            $ph = $candidate;
          }
        }
      }
...
function renderSheetRegionHtml(Worksheet $sheet, string $colStartLetter, string $colEndLetter, int $rowStart, int $rowEnd, ?array $overlay = null): string
{
  ...
  if ($defaultColW === null || $defaultColW <= 0)
    $defaultColW = 8.43;
  ...
  if ($defaultRowPt === -1 || $defaultRowPt === null)
    $defaultRowPt = 15;
  ...
  for ($r = $rowStart; $r <= $rowEnd; $r++) {
    ...
    for ($c = $cStart; $c <= $cEnd; $c++) {
      ...
      if ($bTop)
        $stylePieces[] = "border-top:{$bTop}";
      if ($bRight)
        $stylePieces[] = "border-right:{$bRight}";
      if ($bBottom)
        $stylePieces[] = "border-bottom:{$bBottom}";
      if ($bLeft)
        $stylePieces[] = "border-left:{$bLeft}";
...
function getCellBorderSideCss(Worksheet $sheet, int $row, int $col, string $side): string
{
  ...
  if ($type === Border::BORDER_NONE || !$type)
    return '';
...
function applyJoinRules(array $data, array $joins): array
{
  ...
  foreach ($joins as $fieldPath => $pattern) {
    ...
    $repKey = $parts[0];
    $destKey = $parts[1];
    if (empty($data[$repKey]) || !is_array($data[$repKey]))
      continue;
...
function buildCanonicalData(array $post, ?array $files): array
{
  ...
  for ($i = 0; $i < $N; $i++) {
    ...
    if ($startY . $startM . $name . $fac . $level . $status . $endY . $endM === '')
      continue;
  ...
  for ($i = 0; $i < $E; $i++) {
    ...
    if ($syear . $smon . $comp . $title . $stat . $eyear . $emon === '')
      continue;
