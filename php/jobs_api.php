...
function is_allowed_field($field){
  $allowed = [
    ...
    'salary_basic',
    'salary_takehome',
    'transport_amount_limit',
    'rent_support',

    'current_residence', // ✅ 現在の居住地
    'gender_pref',       // ✅ 性別
    'experience',        // ✅ 経験
    'hijab_policy',      // ✅ ヒジャブ

    'nationality_pref_json', // ✅ 国籍 JSON
    'required_vacancy',
    'japanese_level',
...
try {
  ...
  // ===== staff mapping for existing rows (posted_by username -> staff id) =====
  $staffByUsername = [];
  $staffByName = [];
  try {
  ...
  if ($action === 'list') {
    ...
    if (count($ids) > 0) {
      ...
      $st2 = $pdo->prepare("
        SELECT job_post_id, id, file_path, file_name, mime, created_at
        FROM job_files
        WHERE job_post_id IN ($in)
    ...
    foreach($rows as &$r){
      ...
      // ✅ job_staff_id fallback from posted_by (existing data)
      if (empty($r['job_staff_id'])) {
        $u = (string)($r['posted_by'] ?? '');
        if ($u !== '' && isset($staffByUsername[$u])) {
          $r['job_staff_id'] = $staffByUsername[$u]; // response only (not updating DB)
        } else {
          $r['job_staff_id'] = null;
        }
      } else {
        $r['job_staff_id'] = (int)$r['job_staff_id'];
      }

      // ✅ bonuses -> "あり/なし" (UI)
      $b = $r['bonuses'];
      $r['bonuses'] = ((int)$b === 1) ? "あり" : "なし";

      // ✅ normalize OK variants
      $r['current_residence'] = normalize_ok3($r['current_residence'] ?? '') ?: ($r['current_residence'] ?? null);
      $r['gender_pref'] = normalize_ok3($r['gender_pref'] ?? '') ?: ($r['gender_pref'] ?? null);
      $r['experience'] = normalize_ok3($r['experience'] ?? '') ?: ($r['experience'] ?? null);

      // ✅ nationality JSON -> array
      $arr = [];
      if (!empty($r['nationality_pref_json'])) {
        $decoded = json_decode((string)$r['nationality_pref_json'], true);
        if (json_last_error() === JSON_ERROR_NONE && is_array($decoded)) {
          $arr = array_values(array_filter(array_map('strval', $decoded), fn($x)=>trim($x)!=='' ));
        }
      }
      $r['nationality_pref_json'] = $arr;

      $r['files_preview'] = $filesMap[$jid] ?? [];
    }
  ...
  if ($action === 'create') {
    ...
    $stmt = $pdo->prepare("
      INSERT INTO posts
        (post_type, publish_state, status,
         title, summary, content,
         staff_id, posted_by, job_staff_id,
         company_name,
         org_work_type,
         job_location,
         bonuses,
  ...
  if ($action === 'updateCell') {
    ...
    if ($field === 'status') {
      ...
      $allowed = ['募集中','急募','募集終'];
      if (!in_array($v, $allowed, true)) $v = '募集中';
      $value = $v;
    }

    // job_staff_id: accept id OR staff name (fallback)
    if ($field === 'job_staff_id') {
      if ($value === '' || $value === null) {
        ...
        if (ctype_digit($sv)) {
          ...
          // maybe name came
          $value = $staffByName[$sv] ?? null;
        }
      }
    }

    // bonuses: UI sends "あり/なし" (or 1/0)
    if ($field === 'bonuses') {
      $sv = trim((string)$value);
      if ($sv === 'あり' || $sv === '1' || strtolower($sv) === 'true') $value = 1;
      else $value = 0;
    }
    ...
    if ($field === 'current_residence') {
      ...
      $allowed = ['国内','国外','どちらでもOK', null, ''];
      if (!in_array($v, $allowed, true)) $v = null;
      $value = $v;
    }
    ...
    // publish_state (accept key or label)
    if ($field === 'publish_state') {
      ...
      if (!in_array($v, $allowed, true)) $v = 'draft';
      $value = $v;
    }
    ...
    // return partial updated row (for UI refresh)
    $st2 = $pdo->prepare("
      SELECT
        id,
        status,
    ...
    if ($row) {
      ...
      // bonuses -> "あり/なし"
      $row['bonuses'] = ((int)($row['bonuses'] ?? 0) === 1) ? "あり" : "なし";
      ...
      $row['job_staff_id'] = empty($row['job_staff_id']) ? null : (int)$row['job_staff_id'];
    }
