...
function _normalize_jlpt(?string $s): string
{
  ...
  if ($s === '')
    return '';
  if (preg_match('/N\s*([1-5])/i', $s, $m))
    return 'N' . strtoupper($m[1]);
  if (preg_match('/JFT/i', $s))
    return 'JFT A2';
  if (preg_match('/N([1-5])\s*相当/u', $s, $m))
    return 'N' . $m[1];
  return $s;
}
function _has_experience(array $meta): bool
{
  if (!empty($meta['work_blocks']) && is_array($meta['work_blocks'])) {
    foreach ($meta['work_blocks'] as $r) {
      if (trim(implode('', array_map(fn($v) => (string) $v, $r))) !== '')
        return true;
    }
  }
...
function _is_address_in_japan(string $addr): bool
{
  ...
  if (mb_strpos($a, '日本', 0, 'UTF-8') !== false)
    return true;
  if (stripos($a, 'japan') !== false)
    return true;

  if (preg_match('/\b\d{3}-\d{4}\b/u', $a))
    return true;
  ...
  foreach ($prefs as $p) {
    if (mb_strpos($addr, $p, 0, 'UTF-8') !== false)
      return true;
  }
  return false;
}
...
function _job_title_by_id(PDO $pdo, int $jobId): ?string
{
  ...
  if ($jobId <= 0)
    return null;
  if (isset($_JOB_TITLE_CACHE[$jobId]))
...
function _job_company_by_id(PDO $pdo, int $jobId): ?string
{
  ...
  if ($jobId <= 0)
    return null;
  if (isset($_JOB_COMPANY_CACHE[$jobId]))
...
function _detect_source(array $meta, PDO $pdo): array
{
  ...
  foreach ($srcKeyCandidates as $k) {
    if ($k !== '' && in_array($k, $uploadish, true)) {
      $srcType = 'upload';
      break;
    }
  }
  if ($srcType === 'open' && $origRel !== '')
    $srcType = 'upload';
...
if (is_dir($resumeDir)) {
  ...
  foreach ($list as $jsonPath) {
    ...
    // Grouping key: user_id > email > name
    $uid = (int) ($meta['user_id'] ?? ($meta['_user_id'] ?? 0));
    $email = trim(strtolower((string) ($meta['email'] ?? ($meta['personal']['email'] ?? ''))));
    $groupKey = 'name_' . md5((string) $name);
    if ($uid > 0) {
      $groupKey = 'uid_' . $uid;
    } elseif ($email !== '') {
    ...
    $row_data = [
      ...
      'company_name' => (string) $sd['company_name'],
      'orig_rel' => (string) $sd['orig_rel'],
    ];

    if (!isset($raw_rows[$groupKey]) || $raw_rows[$groupKey]['created'] < $createdAt) {
      $raw_rows[$groupKey] = $row_data;
    }
  }
}
      ...
      <div class="dropdown" data-key="exp">
        ...
        <div class="dropmenu">
          <?php foreach (['yes' => 'あり', 'no' => 'なし'] as $value => $label): ?>
            <label><input type="checkbox" value="<?= htmlspecialchars($value) ?>">
              <span><?= htmlspecialchars($label) ?></span></label>
          <?php endforeach; ?>
          <div class="actions"><button class="btn" onclick="clearMenu(this)">クリア</button><button class="btn"
              onclick="closeMenu(this)">OK</button></div>
        </div>
      </div>
      ...
      <div class="dropdown" data-key="jlpt">
        ...
        <div class="dropmenu">
          <?php foreach (['JFT A2', 'N4', 'N3', 'N2', 'N1'] as $opt): ?>
            <label><input type="checkbox" value="<?= htmlspecialchars($opt) ?>">
              <span><?= htmlspecialchars($opt) ?></span></label>
          <?php endforeach; ?>
          <div class="actions"><button class="btn" onclick="clearMenu(this)">クリア</button><button class="btn"
              onclick="closeMenu(this)">OK</button></div>
        </div>
      ...
      <div class="dropdown" data-key="nat">
        ...
        <div class="dropmenu" style="min-width:260px">
          <?php foreach ($nations as $n): ?>
            <label><input type="checkbox" value="<?= htmlspecialchars($n) ?>">
              <span><?= htmlspecialchars($n) ?></span></label>
          <?php endforeach; ?>
          <div class="actions"><button class="btn" onclick="clearMenu(this)">クリア</button><button class="btn"
              onclick="closeMenu(this)">OK</button></div>
        </div>
      ...
      <tbody>
        ...
        <?php else:
          ...
          foreach ($rows as $r):
            ...
            ?>
            <tr data-nat="<?= htmlspecialchars($r['nat']) ?>" data-exp="<?= htmlspecialchars($r['exp']) ?>"
              data-jlpt="<?= htmlspecialchars($r['jlpt']) ?>" data-res="<?= htmlspecialchars($r['resident']) ?>"
              ...
              <td class="actions">
                <form method="post" action="/php/delete_rireki.php"
                  onsubmit="return confirm('この履歴書を削除しますか？\n対応する Excel / JSON（および原本ファイルがあればそれも）が削除されます。');">
